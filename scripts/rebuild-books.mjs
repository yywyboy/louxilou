...
const META = {
  'kotenbu': {
    title: '古典部系列', author: '米泽穗信', cover: '/books/images/kotenbu/temp.GIF',
    description: '以神山高中古典部为舞台的青春推理小说系列', tags: ['light-novel', 'detective'],
    chapters: [
  ...
  'mobitchi': {
    title: '魔女之旅', author: '白石定规', cover: '/books/images/mobitchi/1.GIF',
    description: '讲述魔女伊蕾娜周游各国的故事', tags: ['light-novel', 'adventure'],
    chapters: [
  ...
  'xiaoshimin': {
    title: '小市民系列', author: '米泽穗信', cover: '/books/images/xiaoshimin/IMG_0106.JPG',
    description: '以小市民为主题的推理小说系列', tags: ['light-novel', 'detective'],
    chapters: [
  ...
  '洛丽塔': { author: '弗拉基米尔·纳博科夫', description: '纳博科夫最具争议的小说，以华丽的文笔讲述一段禁忌之恋', tags: ['classic', 'novel', 'american'] },
...
function scanChapters(bookId) {
  const imgDir = join(BOOKS_DIR, 'images', bookId)
  const txtDir = join(BOOKS_DIR, bookId, 'txt')
  if (!existsSync(txtDir)) return []

  const files = readdirSync(txtDir).filter(f => f.endsWith('.txt')).sort()
  return files.map((f, i) => ({
    id: String(i + 1),
    title: f.replace('.txt', ''),
    cover: existsSync(join(imgDir, 'cover.jpg'))
      ? `/books/images/${bookId}/cover.jpg`
      : `/books/images/${bookId}/${f.replace('.txt', '.jpg')}`,
    status: '已完结',
    txtUrl: `/books/${bookId}/txt/${f}`
  }))
}

const lines = []
lines.push(`export interface BookChapter {`)
lines.push(`  id: string`)
lines.push(`  title: string`)
lines.push(`  cover: string`)
lines.push(`  status: string`)
lines.push(`  txtUrl: string`)
lines.push(`}`)
lines.push(``)
...
lines.push(`  chapters: BookChapter[]`)
lines.push(`}`)
lines.push(``)
lines.push(`export const BOOK_TAGS = [`)
lines.push(`  { id: 'light-novel', name: '轻小说' },`)
lines.push(`  { id: 'classic', name: '经典文学' },`)
lines.push(`  { id: 'novel', name: '长篇小说' },`)
lines.push(`  { id: 'short-story', name: '短篇集' },`)
lines.push(`  { id: 'drama', name: '戏剧' },`)
lines.push(`  { id: 'poetry', name: '诗歌' },`)
lines.push(`  { id: 'fairy-tale', name: '童话' },`)
lines.push(`  { id: 'detective', name: '推理' },`)
lines.push(`  { id: 'adventure', name: '冒险' },`)
lines.push(`  { id: 'romance', name: '爱情' },`)
lines.push(`  { id: 'philosophy', name: '哲学' },`)
lines.push(`  { id: 'dystopia', name: '反乌托邦' },`)
lines.push(`  { id: 'russian', name: '俄国文学' },`)
lines.push(`  { id: 'french', name: '法国文学' },`)
lines.push(`  { id: 'english', name: '英国文学' },`)
lines.push(`  { id: 'american', name: '美国文学' },`)
lines.push(`  { id: 'japanese', name: '日本文学' },`)
lines.push(`  { id: 'german', name: '德国文学' },`)
lines.push(`  { id: 'italian', name: '意大利文学' },`)
lines.push(`  { id: 'spanish', name: '西班牙文学' },`)
lines.push(`  { id: 'latin-american', name: '拉美文学' }`)
lines.push(`]`)
lines.push(``)
lines.push(`export const books: Book[] = [`)

const allBookIds = readdirSync(join(BOOKS_DIR, 'images')).filter(d =>
  existsSync(join(BOOKS_DIR, 'images', d))
)

for (let i = 0; i < allBookIds.length; i++) {
  const bookId = allBookIds[i]
  const meta = META[bookId]
  if (!meta) continue

  const chapters = meta.chapters || scanChapters(bookId)
  const title = meta.title || bookId
  const cover = meta.cover || `/books/images/${bookId}/cover.jpg`
  ...
  for (const ch of chapters) {
    lines.push(`      { id: '${ch.id}', title: '${ch.title.replace(/'/g, "\\'")}', cover: '${ch.cover}', status: '${ch.status}', txtUrl: '${ch.txtUrl}' },`)
  }
...
lines.push(`]`)
lines.push(``)
lines.push(`export function getBookById(id: string): Book | undefined {`)
lines.push(`  return books.find(book => book.id === id)`)
lines.push(`}`)

writeFileSync(BOOKS_TS, lines.join('\n'))
console.log(`Rebuilt books.ts with ${allBookIds.length} books`)
